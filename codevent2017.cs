...
namespace CodeVent2017
{
	class Program
	{
		...
		public class Day7TowerNode
		{
			...
			public Day7TowerNode parentNode = null;
			public List<Day7TowerNode> subTowers = null;

			public Day7TowerNode(string name, int weight)
			{
				this.name = name;
				this.weight = weight;
				subTowerNames = new string[0];
				subTowers = new List<Day7TowerNode>();
			}
		}

		static Day7TowerNode FindAttachPoint(Day7TowerNode rootNode, string nameToFind)
		{
		...
		static void Day7CalcTowerWeights(Day7TowerNode rootNode)
		{
			rootNode.totalChildWeight = rootNode.weight;

			foreach (var subtower in rootNode.subTowers)
			{
				Day7CalcTowerWeights(subtower);
				rootNode.totalChildWeight += subtower.totalChildWeight;
			}
		}

		static Day7TowerNode Day7FindLastUnbalancedNode(Day7TowerNode rootNode)
		{
			...
			{
				return Day7FindLastUnbalancedNode(rootNode.subTowers[unbalancedIndex]);
			}
		}

		static void Day7Puzzle1()
		{
			string filename = "d7p1_b.txt";

			var lines = File.ReadAllLines(filename);
			var rootNode = new Day7TowerNode("", -1); // Just used to keep everything organized
			var allNodes = new List<Day7TowerNode>();
			...
			foreach (var line in lines)
			{
				...
				var tower = new Day7TowerNode(name, weight);
			...
			foreach (var node in allNodes)
			{
				...
				Day7TowerNode attachPoint = null;
				foreach (var tower in rootNode.subTowers)
				{
			...
			Day7CalcTowerWeights(rootNode.subTowers[0]);
			var unbalancedNode = Day7FindLastUnbalancedNode(rootNode.subTowers[0]);
